...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 这里处理的是BeanDefinition，普通业务Bean通常还没有真正实例化
				invokeBeanFactoryPostProcessors(beanFactory);
				// Register bean processors that intercept bean creation.
				// 注册各种bean的后置处理器，后续调用在getBean里面
				// 这里只是注册BeanPostProcessor，真正回调发生在createBean初始化阶段
				...
				// 从这里开始触发剩余非懒加载单例Bean的getBean/createBean流程
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化BeanFactor，并且进行XML文件的读取，将得到的BeanFactory记录到当前实体的属性中
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		if (!shouldIgnoreSpel) {
			// 创建一个StandardBeanExpressionResolver对象，并设置类加载器，解析SpEL表达式的解析器
			beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		}
		// 添加一个资源编辑器注册器，主要是对bean的属性进行编辑 类似我们自定义实现的AddressPropertyEditorRegister
		...
		// 设置自动装配的特殊规则，当在进行ioc初始化的时候，如果有多个实现，使用指定的对象进行注入
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 获取当前程序中所有的BeanFactoryPostProcessor对象
		// 默认情况下获取到的是空集合，如果想扩展怎么实现呢？
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 到这里默认BeanDefinition不会再频繁变化，可以缓存合并后的BeanDefinition元数据
		beanFactory.freezeConfiguration();

		// Instantiate all remaining (non-lazy-init) singletons.
		// 真正批量创建非懒加载单例Bean的入口，继续看DefaultListableBeanFactory#preInstantiateSingletons
